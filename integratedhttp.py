...
class Main_window:
    ...
    def login(self):
        ...
        if not username or not password:
            ...
            return
        try:
            response = self.session.post(f"{SERVER_URL}/login", json={'username': username, 'password': password})
            ...
            messagebox.showerror("Error", "Failed to parse server response.")
    ...
    def register(self):
        ...
        if not username or not password:
            ...
            return
        try:
            response = self.session.post(f"{SERVER_URL}/register", json={'username': username, 'password': password})
            response.raise_for_status()
            data = response.json()
            if 'message' in data:
    ...
    def logout(self):
        try:
            response = self.session.post(f"{SERVER_URL}/logout", json={})
            response.raise_for_status()
            data = response.json()
            if 'message' in data:
    ...
    def view_logs(self):
        try:
            response = self.session.get(f"{SERVER_URL}/logs")
            response.raise_for_status()
            data = response.json()
            self.init_logs_frame(data)
        except requests.exceptions.RequestException as e:
            logging.error(f"HTTP Request failed: {e}")
            messagebox.showerror("Error", "Failed to communicate with server.")
        except requests.exceptions.JSONDecodeError as e:
    ...
    def view_history(self):
        try:
            response = self.session.get(f"{SERVER_URL}/quiz_history")
            response.raise_for_status()
            data = response.json()
            self.init_history_frame(data)
        except requests.exceptions.RequestException as e:
            logging.error(f"HTTP Request failed: {e}")
            messagebox.showerror("Error", "Failed to communicate with server.")
        except requests.exceptions.JSONDecodeError as e:
    ...
    def add_question(self):
        ...
        try:
            response = self.session.post(f"{SERVER_URL}/add_question", json={
                'subject': subject,
            ...
            if 'message' in data:
                ...
                self.question_entry.insert("1.0", "E.g. What is the value of sine of 90 degrees?")
                self.question_entry.configure(foreground="gray")
                self.choice_entry.delete("1.0", tk.END)
                self.choice_entry.insert("1.0", "E.g. 0,1,-1,2")
                self.choice_entry.configure(foreground="gray")
                self.correct_answer_entry.delete("1.0", tk.END)
                self.correct_answer_entry.insert("1.0", "E.g. 1")
                self.correct_answer_entry.configure(foreground="gray")
            else:
    ...
    def start_quiz(self):
        ...
        try:
            response = self.session.get(f"{SERVER_URL}/get_questions", params={'subject': subject})
            response.raise_for_status()
            data = response.json()
            self.questions = data
    ...
    def submit_quiz(self):
        ...
        try:
            response = self.session.post(f"{SERVER_URL}/submit_answers", json={'answers': self.submitted_answers})
            response.raise_for_status()
            data = response.json()
            if 'message' in data:
                messagebox.showinfo("Quiz Completed", f"You answered {data['correct_count']} questions correctly!")
                self.init_student_role_frame()
            else:
                messagebox.showerror("Error", data["error"])
        except requests.exceptions.RequestException as e:
